...
const fetchProducts = async (page = 1, limit = 20): Promise<ApiResponse> => {
...
export default function AdminProductsPage() {
  ...
  // Group products by coach and filter by search query
  const coachGroups = useMemo(() => {
    const groups: Map<string, CoachGroup> = new Map();
    
    products.forEach((product) => {
      const coachId = product.coachId?._id || "unknown";
      const coachName = product.coachId?.fullName || "Unknown Coach";
      const coachEmail = product.coachId?.email || "";
      ...
      groups.get(coachId)!.products.push(product);
    });
    ...
    <div>
      ...
      <section>
        ...
        {!isLoading && !error && coachGroups.length === 0 && (
          <div className="admin-card" style={{ textAlign: 'center', padding: '3rem', color: 'var(--admin-color-muted)' }}>
            {searchQuery ? "No coaches found matching your search" : "No products found"}
          </div>
        )}

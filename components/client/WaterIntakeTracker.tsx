...
export default function WaterIntakeTracker() {
  ...
  const logMutation = useMutation({
    mutationFn: async () => {
      ...
      const res = await api.post("/water-intake", {
        amountLiters: parseFloat(amountInput),
        date: new Date(selectedDate),
        goal: parseFloat(goalInput) || 3.5,
  ...
  const todayGoal = todayData?.data?.goal || 3.5;
  const todayPercentage = ((todayAmount / todayGoal) * 100).toFixed(1);
      ...
      <section style={{ marginTop: "1.25rem" }}>
        <div className="admin-card">
          ...
          <div style={{ marginTop: "0.75rem" }}>
            ...
            >
              {notesInput === "" ? "+ Add Notes" : "- Hide Notes"}
            </button>
          </div>
        </div>
      </section>
